...
public class AddPotionMixRecipe implements PotionBrewingRecipe {
    ...
    public void addPotionMixes(List<PotionBrewing.Mix<Potion>> mixes) {
        ...
        if (item == null) {
            mix = new PotionBrewing.Mix<Potion>(ForgeRegistries.POTIONS,input, Ingredient.of(itemTagKey), result);
        } else {
            mix = new PotionBrewing.Mix<Potion>(ForgeRegistries.POTIONS,input, Ingredient.of(item), result);
        }
