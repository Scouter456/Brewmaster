...
public class AddContainerMixRecipe implements PotionBrewingRecipe{
    ...
    public void addContainerMixes(List<PotionBrewing.Mix<Item>> mixes) {
        ...
        if (item == null) {
            mix = new PotionBrewing.Mix<Item>(ForgeRegistries.ITEMS,input, Ingredient.of(itemTagKey), result);
        } else {
            mix = new PotionBrewing.Mix<Item>(ForgeRegistries.ITEMS,input, Ingredient.of(item), result);
        }
