...
public class ReplacePotionMixRecipe implements PotionBrewingRecipe {
    ...
    public void addPotionMixes(List<PotionBrewing.Mix<Potion>> mixes) {
        ...
        if (!foundRecipe) {
            LOGGER.warn("Brewmaster: replace_potion_mix did not find old recipe with input {}, ingredient {}, result {}", oldRecipe.input(), oldRecipe.ingredient(), oldRecipe.result());
        }
    }
    ...
    public PotionBrewingRecipeType<? extends PotionBrewingRecipe> type() {
        return BMPotionRecipeRegistry.REPLACE_POTION_MIX.get();
    }
